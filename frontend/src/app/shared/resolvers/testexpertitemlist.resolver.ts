...
export class TestExpertItemListResolver implements Resolve<Array<TestExportListItem>> {
    constructor(private itemService: ItemService, private userService: UserService, private translateService: TranslateService) { }

    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<Array<TestExportListItem>> {
        const menuItem = +MenuItemTestExpert[route.params.menuItem] as MenuItemTestExpert;
